 ...
 * @LastEditTime: 2020-08-25 17:16:45
 * @Description: 左侧菜单栏
 */
      ...
      <ScrollView>
        <ul className="menu-list">
          {this.state.menuList.map((item, index) => {
            return (
              item.path ?
                <NavLink onClick={this.handelIndex.bind(this, index)} activeClassName="menu-list-active" key={index} to={item.path}>
                  <li> {item.name}</li>
                </NavLink>
                :
